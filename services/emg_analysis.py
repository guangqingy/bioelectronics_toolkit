...
from services import emg_analysis_processing
...
class EmgAnalysisService:
    """Payload builders for the EMG analysis Web API."""
    ...
    def plot_payload(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        dsf = emg_analysis_processing.downsample_factor(downsample, len(t))
        t_d = t[::dsf]
        y_d = y[::dsf]
        ...
        try:
            ...
            emg_analysis_processing.finish_axis(ax, t_d, y_min, y_max, grid=fig_params["show_grid"])
            fig.tight_layout()
            return {
                "img": self.fig_to_b64(fig),
                "downsample": dsf,
                "plotted_points": int(len(t_d)),
                "inverted_y": emg_analysis_processing.y_inversion_enabled(data),
            }
        finally:
            close_figure(fig)
    ...
    def export_channel_payload(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if fmt == "csv":
            ...
            if self.mode_is_save(mode):
                ...
                return self._save_result(out_path, "emg_analysis_channel_csv")
        ...
        if fmt == "svg":
            ...
            if self.mode_is_save(mode):
                ...
                out_path.write_bytes(payload)
                return self._save_result(out_path, "emg_analysis_channel_svg")
    ...
    def export_processing_payload(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        stem = sanitize_name_part(f"{out_stem}_{ch_name}_{result.kind}", "emg_analysis_processing")
        try:
            if fmt == "csv":
                payload = emg_analysis_processing.dataframe_csv_bytes(result.table)
                mimetype = "text/csv"
            else:
                fig_params = emg_analysis_processing.figure_params(
                    data, default_line_color=self.line_color, default_show_title=False
                )
                payload = emg_analysis_processing.figure_bytes(
                    result.figure, fmt, dpi=fig_params["dpi"]
                )
                mimetype = "image/png" if fmt == "png" else "image/svg+xml"
        finally:
            close_figure(result.figure)
        ...
        if self.mode_is_save(mode):
            out_path = self.next_numbered_path(src.with_name(filename))
            out_path.write_bytes(payload)
            outputs = [self._output(out_path, f"emg_analysis_processing_{fmt}")]
            data_out = {
                "ok": True,
    ...
    def export_queue_payload(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "warnings": warnings,
            "outputs": [self._output(path, "emg_analysis_queue_export") for path in saved_paths],
        }
    ...
    def _filtered_export_view(self, data: dict[str, Any], t, fs, y):
        ...
        y_view = emg_analysis_processing.apply_y_polarity(y_view, data)
        dsf = emg_analysis_processing.downsample_factor(
    ...
    def _processing_result(self, data: dict[str, Any]):
        ...
        result = emg_analysis_processing.process_trace(
            t, y, fs, data, default_line_color=self.line_color
        )
        result.metadata["inverted_y"] = emg_analysis_processing.y_inversion_enabled(data)
    ...
    def _save_all_channels(
        ...
        outputs = [self._output(target_dir, "emg_analysis_channel_folder")]
        outputs.extend(self._output(path, "emg_analysis_channel_csv") for path in saved_paths[1:])
        return {
            "kind": "save",
            "data": {
                "ok": True,

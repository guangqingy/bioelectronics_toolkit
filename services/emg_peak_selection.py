...
# EMG traces are dense high-frequency waveforms. The generic 4k-point
...
class EmgPeakSelectionService:
    """Load, detect, plot, and export EMG peak-selection data."""
    ...
    def _read_csv(path: str | Path) -> pd.DataFrame:
        ...
        if not path_text:
            raise ValueError("Missing EMG CSV path")
        csv_path = Path(path_text).expanduser()
        if not csv_path.is_file():
            raise ValueError(f"EMG CSV file not found: {csv_path}")
        ...
        try:
            ...
            raise ValueError(f"Could not read EMG CSV file: {csv_path}: {exc}") from exc

    def load_duration_payload(self, folder: str, subfolder: str, channel: str) -> dict[str, float]:
        path = Path(folder) / subfolder / channel
    ...
    def grouped_export_payload(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.mode_is_save(mode):
            ...
            out_path = (
                ...
                if src is not None
                else resolve_output_dir(default_suffix="emg") / "emg_peak_selection.csv"
            )
            ...
            return {
                ...
                "data": {
                    ...
                    "outputs": self._peak_outputs(out_path, "emg_peak_selection_csv"),
                },
            }
        return {
            "kind": "download",
            "payload": payload,
            "mimetype": "text/csv",
            "download_name": "emg_peak_selection.csv",
        }
        ...
        if self.mode_is_save(mode):
            ...
            return {
                ...
                "data": {
                    "ok": True,
                    "saved_path": str(out_path),
                    "outputs": self._peak_outputs(out_path, "emg_peak_selection_grouped_csv"),
                },
            }
        return {
            "kind": "download",
    ...
    def _load_signal(
        ...
        df = EmgPeakSelectionService._read_csv(path)
        t_col, v_col = emg_service.pick_columns(df)
        t_raw, v_raw, valid = emg_service.numeric_signal(df, t_col, v_col)
        if invert_signal:
            v_raw = -v_raw

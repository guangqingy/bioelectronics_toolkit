...
class RhdViewerService:
    ...
    def plot_payload(self, data: dict[str, Any]) -> dict[str, Any]:
        self._require_rhd()
        t, _fs, _ch_names, y, ch, ch_label, base_stem, used_pair, _segment_count = self._load_view_trace(data)
        ...
        fig_params = rhd_processing.figure_params(data, default_line_color=self.line_color, default_show_title=True)
        dsf = rhd_processing.downsample_factor(downsample, len(t))
        t_d = t[::dsf]
        y_d = y[::dsf]

        fig, ax = plt.subplots(figsize=(fig_params["width_in"], fig_params["height_in"]), dpi=fig_params["dpi"])
        try:
            ax.plot(t_d, y_d, color=fig_params["line_color"], lw=fig_params["line_width"])
            ax.set_xlabel("Time (s)")
            ax.set_ylabel("Amplitude (uV)")
            if fig_params["show_title"]:
                title_name = f"{base_stem} (merged)" if used_pair else Path(data.get("path", "")).name
    ...
    def export_channel_payload(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        fig_params = rhd_processing.figure_params(data, default_line_color=self.line_color, default_show_title=False)
    ...
    def export_all_payload(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        t_all, _fs, ch_all, amp_all, base_stem, _used_pair = rhd_service.load_with_merge_option(src, self.rhd_module, do_merge)
        ...
        with zipfile.ZipFile(out, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            for i, name in enumerate(ch_all):
                zf.writestr(
                    ...
                    pd.DataFrame({"time_s": t_all, "value_uV": amp_all[i, :]}).to_csv(index=False).encode("utf-8"),
                )
        return self._download_result(out.getvalue(), "application/zip", f"{base_stem}_all_channels.zip")
        ...
        for raw in paths:
            ...
            try:
                ...
                if do_merge and recording_key in processed_recordings:
                    continue
                t_all, _fs, ch_all, amp_all, base_stem, _used_pair = rhd_service.load_with_merge_option(p, self.rhd_module, do_merge)
    ...
    def _load_view_trace(self, data: dict[str, Any]):
        ...
        do_merge = self._merge_enabled(data, default=False)
        t, fs, ch_names, y, ch, ch_label, base_stem, used_pair, segment_count = rhd_service.load_channel_with_merge_option(
            path, self.rhd_module, ch_in, do_merge
        )
        t, y = rhd_processing.apply_time_window(t, y, self.float_or(data.get("x_min"), None), self.float_or(data.get("x_max"), None))
    ...
    def _load_export_trace(self, data: dict[str, Any]):
        src = Path(str(data.get("path", "") or "").strip())
        t, fs, _ch_names, y, _ch, ch_name, base_stem, used_pair, _segment_count = rhd_service.load_channel_with_merge_option(
            src, self.rhd_module, data.get("channel", 0), self._merge_enabled(data, default=False)
        )
    ...
    def _filtered_export_view(self, data: dict[str, Any], t, fs, y):
        t_view, y_view = rhd_processing.apply_time_window(t, y, self.float_or(data.get("x_min"), None), self.float_or(data.get("x_max"), None))
        y_view = rhd_processing.apply_filter(y_view, fs, rhd_processing.filter_params(data))
        dsf = rhd_processing.downsample_factor(data.get("downsample", data.get("dsf", "auto")), len(t_view))
        return t_view[::dsf], y_view[::dsf]

    def _processing_result(self, data: dict[str, Any]):
        src = Path(str(data.get("path", "") or "").strip())
        t, fs, _ch_names, y, _ch, ch_name, base_stem, used_pair, _segment_count = self._load_view_trace(data)
    ...
    def _save_all_channels(self, src: Path, base_stem: str, t_all, ch_all, amp_all, wide_csv: bool) -> dict[str, Any]:
        saved_paths = self._write_all_channels(src, base_stem, t_all, ch_all, amp_all, wide_csv)
        if wide_csv:
            out_path = Path(saved_paths[0])
            return {"kind": "save", "data": {"ok": True, "saved_path": str(out_path), "saved_paths": saved_paths, "outputs": [self._output(out_path, "rhd_all_channels_wide")]}}
        ...
        return {"kind": "save", "data": {"ok": True, "saved_path": str(target_dir), "saved_count": len(saved_paths) - 1, "saved_paths": saved_paths[1:], "outputs": outputs}}

    def _write_all_channels(self, src: Path, base_stem: str, t_all, ch_all, amp_all, wide_csv: bool) -> list[str]:
        if wide_csv:
            out_path = src.parent / f"{base_stem}.csv"
            rhd_service.all_channels_wide_frame(t_all, ch_all, amp_all).to_csv(out_path, index=False, sep="\t")
            return [str(out_path)]
    ...
    def _output(path: str | Path, role: str = "rhd_export") -> dict[str, str]:
        p = Path(path)
        return {"path": str(p), "type": "directory" if p.is_dir() else (p.suffix.lower().lstrip(".") or "file"), "role": role}

    def _save_result(self, out_path: Path, role: str) -> dict[str, Any]:
        outputs = [self._output(out_path, role)]
        return {"kind": "save", "data": {"ok": True, "saved_path": str(out_path), "saved_paths": [str(out_path)], "outputs": outputs}, "outputs": outputs}

    @staticmethod
    def _download_result(payload: bytes, mimetype: str, filename: str, **extra) -> dict[str, Any]:
        return {"kind": "download", "payload": payload, "mimetype": mimetype, "download_name": filename, **extra}

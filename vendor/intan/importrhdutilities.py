...
def read_header(fid):
    ...
    freq["desired_impedance_test_frequency"], freq["actual_impedance_test_frequency"] = (
...
def read_one_data_block(data, header, indices, fid):
    ...
    if (header["version"]["major"] == 1 and header["version"]["minor"] >= 2) or (
        ...
        data["t_amplifier"][
            indices["amplifier"] : (indices["amplifier"] + header["num_samples_per_data_block"])
        ] = np.array(
            struct.unpack(
    ...
    if header["num_amplifier_channels"] > 0:
        tmp = np.fromfile(
            fid,
            dtype="uint16",
            count=header["num_samples_per_data_block"] * header["num_amplifier_channels"],
        )
        data["amplifier_data"][
            range(header["num_amplifier_channels"]),
            (indices["amplifier"]) : (indices["amplifier"] + header["num_samples_per_data_block"]),
        ] = tmp.reshape(header["num_amplifier_channels"], header["num_samples_per_data_block"])
    ...
    if header["num_aux_input_channels"] > 0:
        tmp = np.fromfile(
            ...
            dtype="uint16",
            count=int(
                (header["num_samples_per_data_block"] / 4) * header["num_aux_input_channels"]
            ),
        )
        data["aux_input_data"][
            ...
            header["num_aux_input_channels"], int(header["num_samples_per_data_block"] / 4)
        )
    ...
    if header["num_board_adc_channels"] > 0:
        tmp = np.fromfile(
            fid,
            dtype="uint16",
            count=(header["num_samples_per_data_block"]) * header["num_board_adc_channels"],
        )
        data["board_adc_data"][
            range(header["num_board_adc_channels"]),
            indices["board_adc"] : (indices["board_adc"] + header["num_samples_per_data_block"]),
        ] = tmp.reshape(header["num_board_adc_channels"], header["num_samples_per_data_block"])
...
def data_to_result(header, data, data_present):
    ...
    if (
        header["num_board_dig_in_channels"] > 0 or header["num_board_dig_out_channels"] > 0
    ) and data_present:
...
def load_file(filename):
    ...
    if data_present:
        ...
        data["board_dig_in_data"] = np.zeros(
            [header["num_board_dig_in_channels"], num_board_dig_in_samples], dtype=bool
        )
        data["board_dig_in_raw"] = np.zeros(num_board_dig_in_samples, dtype=np.uint)

        data["board_dig_out_data"] = np.zeros(
            [header["num_board_dig_out_channels"], num_board_dig_out_samples], dtype=bool
        )
...
def main():
    print(
        "importrhdutilities.py is a helper module for reading Intan RHD files. "
        "Import it from a viewer or analysis script instead of running it directly."
    )

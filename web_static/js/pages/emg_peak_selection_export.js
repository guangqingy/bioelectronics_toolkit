function emgPeakSelectionOutputBasename(path) {
  return String(path || '').split(/[\\/]/).pop() || String(path || '');
}

function renderEmgPeakSelectionExportOutputs(data) {
  ...
  summary.textContent = `Saved ${segN} segment file(s)`
    ...
    + (summaryPath ? ` · summary: ${emgPeakSelectionOutputBasename(summaryPath)}` : '');
  list.innerHTML = '';
...
function exportGrouped() {
  ...
  const invertSignal = typeof isEmgSignalInverted === 'function' ? isEmgSignalInverted() : false;

  dpRunJobEndpoint('/api/emg/peak-selection/export_job', {
...
[
  'exportGrouped',
  'renderEmgPeakSelectionExportOutputs',
].forEach(name => {

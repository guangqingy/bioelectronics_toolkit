...
function updateEmgAnalysisParameterGroups() {
...
function scanFolder(options) {
  ...
  api('/api/emg/analysis/browse', { folder })
    .then(data => {
      if (data.error) throw new Error(data.error);
      const filePaths = (data.files || []).map((f) => typeof f === 'string' ? f : (f.path || ''));
      _emgAnalysisFiles = filePaths.slice();
      if (opts.selectedPath && _emgAnalysisFiles.includes(opts.selectedPath)) {
...
function renderEmgAnalysisFileList(options) {
  const opts = options || {};
  const list = document.getElementById('emgAnalysisFileList');
  list.innerHTML = '';
  ...
  if (previewMergeEnabled()) {
    ...
    item.textContent = (baseName(folder) || 'Folder') + ' recording (' + _emgAnalysisFiles.length + ' files)';
    item.title = _emgAnalysisFiles.join('\n');
    item.onclick = () => selectFile(item, _emgAnalysisFiles[0], {
      preserveChannel: true,
      loadProfile: false,
    });
    list.appendChild(item);
    selectFile(item, _emgAnalysisFiles[0], opts);
    return;
  }

  _emgAnalysisFiles.forEach(path => {
    const item = document.createElement('div');
    item.className = 'file-item';
  ...
  const selectedPath = (_currentFile && _emgAnalysisFiles.includes(_currentFile)) ? _currentFile : _emgAnalysisFiles[0];
...
function queueAddCurrent() {
  ...
  const paths = currentEmgAnalysisBatchPaths();
...
function queueAddAll() {
  const added = queueAddPaths(_emgAnalysisFiles);
...
function queueAddAllRecursive() {
  ...
  api('/api/emg/analysis/browse_recursive', { folder })
    .then(data => {
      if (data.error) throw new Error(data.error);
...
function emgAnalysisExportParams(extra) {
  return Object.assign({
...
function exportQueue() {
  ...
  dpRunJobEndpoint('/api/emg/analysis/export_queue_job', payload, {
    interval_ms: 1000,
    on_update: job => {
...
[
  ...
  'queueRender',
  'renderEmgAnalysisFileList',
  'emgAnalysisExportParams',
  'scanFolder',
  'updateEmgAnalysisParameterGroups',
  'viewNumber',
].forEach(name => {

...
| RHD viewer | `bte-rhd-viewer` (`bte-emg-viewer` alias) | `/emg/rhd`, `web_api/rhd_viewer.py` | Channel naming, channel resolution, paired split-file merge, and wide export table helpers are service-backed. Web adds recursive browse, jobs, profiles, and manifests. |
